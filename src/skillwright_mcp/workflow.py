...
WORKFLOW_SCHEMA_VERSION: Literal[4] = 4
PrimitiveType = Literal["string", "number", "integer", "boolean"]
PrimitiveValue = str | float | int | bool
_TEMPLATE_RE = re.compile(r"{{\s*([A-Za-z_][A-Za-z0-9_]*)\s*}}")
_IDENTIFIER_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")


def validate_public_input_name(name: str) -> None:
    if _IDENTIFIER_RE.fullmatch(name) is None:
        raise ValueError("workflow input name must be a valid identifier")
    if keyword.iskeyword(name) or name == "ctx" or name.startswith("_"):
        raise ValueError(f"workflow input name {name!r} is reserved for MCP tool generation")
...
class WorkflowInput(StrictModel):
    ...
    def validate_input(self) -> WorkflowInput:
        ...
        if self.default is not None:
            _coerce_input("default", self, self.default)
        return self
...
class WorkflowOutput(StrictModel):
    type: PrimitiveType = "string"
    description: str | None = None
    source: str | None = Field(default=None, pattern=r"^[A-Za-z_][A-Za-z0-9_]*$")
...
class SkillCallStep(StrictModel):
    """Invoke one immutable version of another saved Skillwright skill."""

    op: Literal["skill"] = "skill"
    ...
    @model_validator(mode="after")
    def validate_output_names(self) -> SkillCallStep:
        invalid = [name for name in self.outputs.values() if _IDENTIFIER_RE.fullmatch(name) is None]
        if invalid:
            raise ValueError("composed output targets must be valid workflow identifiers")
        return self
...
class WorkflowDefinition(StrictModel):
    schema_version: Literal[1, 2, 3, 4] = WORKFLOW_SCHEMA_VERSION
    ...
    def validate_definition(self) -> WorkflowDefinition:
        invalid_names = [name for name in self.inputs if _IDENTIFIER_RE.fullmatch(name) is None]
        invalid_outputs = [name for name in self.outputs if _IDENTIFIER_RE.fullmatch(name) is None]
        if invalid_names or invalid_outputs:
            raise ValueError(
                "workflow input/output names must be valid template identifiers: "
                + ", ".join(sorted({*invalid_names, *invalid_outputs}))
            )
        invalid_tool_inputs = []
        for name in self.inputs:
            try:
                validate_public_input_name(name)
            except ValueError:
                invalid_tool_inputs.append(name)
        if invalid_tool_inputs:
            raise ValueError(
                "workflow input names must be callable MCP parameter names; reserved names: "
                + ", ".join(sorted(invalid_tool_inputs))
            )
        ...
        for step_index, step in enumerate(self.steps):
            ...
            for secret_name in secret_names:
                for path, value in _string_leaves(dumped):
                    if secret_name not in template_names(value):
                        continue
    ...
    def public_inputs(self) -> dict[str, WorkflowInput]:
        return {name: spec for name, spec in self.inputs.items() if not spec.secret}
...
def render_value(value: PrimitiveValue, variables: dict[str, Any]) -> Any:
    """Render composition bindings while preserving exact-template primitive types."""
...
def template_name(value: str) -> str | None:
